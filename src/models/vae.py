...
class VAEEncoder(nn.Module):
    def __init__(self, input_dim=49, hidden_dim=256, latent_dim=32, num_layers=2):
    ...
    def forward(self, x):
        ...
        log_var = self.fc_logvar(hidden_concat)
        log_var = torch.log(F.softplus(log_var) + 1e-8)
        
        return mu, log_var


class VAEDecoder(nn.Module):
    def __init__(self, latent_dim=32, hidden_dim=256, output_dim=49, seq_len=128, num_layers=2):
...
class VAE(nn.Module):
    def __init__(self, input_dim=49, hidden_dim=256, latent_dim=32, seq_len=128, num_layers=2):
